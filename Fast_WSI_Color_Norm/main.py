...
if op==0:
	...
	input_direc="F:/git/github/Project/Fast_WSI_Color_Norm/dye"
	output_direc="F:/git/github/Project/Fast_WSI_Color_Norm/new_dye"
	if not os.path.exists(output_direc):
		os.makedirs(output_direc)
	file_type="*.*"
	#file_type="*.svs" #all of these file types from input_direc will be normalized
	target_filename="F:/git/github/Project/Fast_WSI_Color_Norm/40_2_1.jpg"
	if not os.path.exists(target_filename):
		print ("Target file does not exist")
		sys.exit()
	if len(sorted(glob.glob(input_direc+file_type)))==0:
